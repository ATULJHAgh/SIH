    ...
    const fetchWeather = async () => {
      try {
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${temple.lat}&lon=${temple.lng}&units=metric&appid=${WEATHER_API_KEY}`
        );
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <p className="text-gray-300 mb-2">{temple.description}</p>
          {temple.history && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">History:</span> {temple.history}
            </p>
          )}
          {temple.culturalOverview && (
            <p className="text-gray-400 mb-2">
          ...
          {temple.importance && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Importance:</span> {temple.importance}
            </p>
          )}
          {temple.bestTime && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Best Time:</span> {temple.bestTime}
            </p>
          )}
          {/* Categories */}
          {temple.categories && (
            <div className="flex flex-wrap gap-2 mt-4">
              {temple.categories.map((cat, idx) => (
                <span
                  key={idx}
                  className="bg-green-700 px-3 py-1 rounded-full text-sm font-semibold"
                >
                  {cat}
                </span>
              ))}
      ...
      {/* Gemini Chatbot */}
      <div className="max-w-6xl mx-auto px-6 py-12">
        <GeminiChatBot spotId={id} spotType="temple" />
      </div>
