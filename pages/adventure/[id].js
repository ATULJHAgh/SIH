...
import adventures from "@/data/adventures.json"; // Updated JSON with lat/lng, fees, animals
...
export default function AdventureDetail() {
  ...
  // Fetch weather
  useEffect(() => {
    if (!adventure?.lat || !adventure?.lng) return;

    const fetchWeather = async () => {
      try {
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${adventure.lat}&lon=${adventure.lng}&units=metric&appid=${WEATHER_API_KEY}`
        );
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <p className="text-gray-300 mb-2">{adventure.description}</p>
          {adventure.longDescription && (
            <p className="text-gray-400 mb-2">{adventure.longDescription}</p>
          )}
          {adventure.history && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">History:</span> {adventure.history}
            </p>
          )}
          {adventure.importance && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Importance:</span> {adventure.importance}
            </p>
          )}
          {adventure.bestTime && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Best Time:</span> {adventure.bestTime}
            </p>
          )}
          {adventure.fees && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Entry Fees:</span> {adventure.fees}
            </p>
          )}
          {adventure.thingsToCarry && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Things to Carry:</span> {adventure.thingsToCarry}
            </p>
          )}
          {adventure.speciality && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Speciality:</span> {adventure.speciality}
            </p>
          )}
          {adventure.animals && adventure.animals.length > 0 && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Animals:</span> {adventure.animals.join(", ")}
            </p>
          )}
        </div>
      </div>
      ...
      {adventure.lat && adventure.lng && (
        <div className="max-w-6xl mx-auto px-6 py-6 relative z-0 mb-6 border-4 border-green-400 rounded-lg">
          <MapContainer
            center={[adventure.lat, adventure.lng]}
            zoom={10}
            scrollWheelZoom={true}
            style={{ height: "400px", width: "100%" }}
          >
            <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
            <Marker position={[adventure.lat, adventure.lng]} />
          </MapContainer>
        </div>
      )}
      ...
      {/* Gemini Chatbot */}
      <div className="max-w-6xl mx-auto px-6 py-12">
        <GeminiChatBot spotId={id} spotType="adventure" />
      </div>
