...
export default function MonumentDetail() {
  ...
  useEffect(() => {
    if (!id) return;
    const foundMonument = monuments.find((m) => m.id === id);
    ...
    const fetchWeather = async () => {
      try {
        const res = await fetch(
          `https://api.openweathermap.org/data/2.5/weather?lat=${monument.lat}&lon=${monument.lng}&units=metric&appid=${WEATHER_API_KEY}`
        );
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-12 items-center">
        ...
        <div>
          ...
          <p className="text-gray-300 mb-2">{monument.description}</p>
          {monument.history && (
          ...
          {monument.importance && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Importance:</span> {monument.importance}
            </p>
          )}
          {monument.bestTime && (
            <p className="text-gray-400 mb-2">
              <span className="font-semibold">Best Time:</span> {monument.bestTime}
            </p>
          )}
          {/* Categories */}
          {monument.category && (
      ...
      {monument.lat && monument.lng && (
        <div className="max-w-6xl mx-auto px-6 py-6 relative z-0 mb-6 border-4 border-green-400 rounded-lg">
          ...
          >
            ...
            <Marker position={[monument.lat, monument.lng]} />
          </MapContainer>
        </div>
      )}
      ...
      {/* Gemini Chatbot */}
      <div className="max-w-6xl mx-auto px-6 py-12">
        <GeminiChatBot spotId={id} spotType="monument" />
      </div>
