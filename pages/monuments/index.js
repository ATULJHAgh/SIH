...
export default function MonumentsPage() {
  return (
    <div
      ...
      style={{ backgroundImage: "url('/images/monuments/bg.jpg')" }}
    >
      {/* Navbar */}
      <Navbar />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {monuments.map((monument) => (
            <Link key={monument.id} href={`/monuments/${monument.id}`}>
              <div className="bg-gray-800 bg-opacity-90 rounded-2xl shadow-lg overflow-hidden cursor-pointer transform transition hover:scale-105 hover:shadow-2xl flex flex-col h-[280px]">
                <img
                  src={monument.image}
                  alt={monument.name}
